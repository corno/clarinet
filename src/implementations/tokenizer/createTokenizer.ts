...
export function createTokenizer(
    ...
    function setCurrentToken(contextType: CurrentToken, range: Range) {
    ...
    function unsetCurrentToken(range: Range) {
        if (currentToken[0] === CurrentTokenType.NONE) {
    ...
    return {
        onData: (data: PreToken): p.IValue<boolean> => {
            switch (data.type[0]) {
                ...
                case PreTokenDataType.LineCommentEnd: {
                    const $ = data.type[1]
                    function onLineCommentEnd(location: Location): p.IValue<boolean> {
                ...
                case PreTokenDataType.NewLine: {
                    const $ = data.type[1]
                    function onNewLine(_range: Range, _tokenString: string): p.IValue<boolean> {
                ...
                case PreTokenDataType.WrappedStringBegin: {
                    const $ = data.type[1]
                    indentationState.setLineDirty()
                    function onWrappedStringBegin(begin: Range, quote: WrappedStringType): p.IValue<boolean> {
                        setCurrentToken(
                ...
                case PreTokenDataType.WrappedStringEnd: {
                    const $ = data.type[1]
                    function onWrappedStringEnd(end: Range, wrapper: string | null): p.IValue<boolean> {
                ...
                case PreTokenDataType.NonWrappedStringBegin: {
                    const $ = data.type[1]
                    function onNonWrappedStringBegin(location: Location): p.IValue<boolean> {
                ...
                case PreTokenDataType.NonWrappedStringEnd: {
                    const $ = data.type[1]
                    function onNonWrappedStringEnd(location: Location): p.IValue<boolean> {
                ...
                case PreTokenDataType.WhiteSpaceBegin: {
                    const $ = data.type[1]
                    function onWhitespaceBegin(location: Location): p.IValue<boolean> {
                ...
                case PreTokenDataType.WhiteSpaceEnd: {
                    const $ = data.type[1]
                    function onWhitespaceEnd(location: Location): p.IValue<boolean> {
        ...
        onEnd: (aborted: boolean, location: Location): p.IValue<null> => {
            parser.onEnd(
                aborted,
                createAnnotation(
