...
import { ExpectErrorValueType } from "./functionTypes"

function assertUnreachable<RT>(_x: never): RT {
    throw new Error("unreachable")
}

export function printExpectErrorValueType(vt: i.ExpectError): string {
    switch (vt) {
        case "array": {
            return `an array ([] or <>)`
        }
...
export function printExpectError(issue: ExpectErrorValueType): string {
    switch (issue[0]) {
