import { FlattenedHandler, StackContext } from "../../../interfaces/untyped"
import { FormatInstructionWriter } from "./FormatInstruction"
import {
    createSerializedApostrophedString,
    createSerializedMultilineString,
...
export function createASTNNormalizer<TokenAnnotation, NonTokenAnnotation>(
    ...
    newline: string,
    writer: FormatInstructionWriter<TokenAnnotation, NonTokenAnnotation>,
): FlattenedHandler<TokenAnnotation, NonTokenAnnotation> {
    ...
    return {
            ...
            writer.token(
                {
                    ...
                    token: ((): string => {
                        switch ($.objectToken.data.type[0]) {
                            case "verbose group": {
                                return createSerializedApostrophedString($.propertyToken.data.value)
                            }
                            case "dictionary": {
                                return createSerializedQuotedString($.propertyToken.data.value)
                            }
                            default:
        ...
        multilineStringValue: $ => {
            writer.token(
                {
                    stringBefore: ``,
                    token: createSerializedMultilineString(
                        $.token.data.lines,
                        createIndentation($.stackContext),
                    ),
                    stringAfter: ``,
        ...
        option: $ => {
            writer.token(
                {
                    stringBefore: ``,
                    token: createSerializedApostrophedString($.token.data.value),
                    stringAfter: ` `,
                },
                $.token.annotation,
            )
